  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (error.name === 'TypeError' && error.message.includes('fetch')) {
        errorMessageText = 'Sorry, I cannot connect to the AI service. Please make sure the backend server is running on port 8001.';
      } else if (error.name === 'TypeError' && error.message.includes('Failed to fetch')) {
        errorMessageText = 'Sorry, I encountered a network error. The backend server may be down or not responding.';
          ...
          <div className={styles.chatMessages}>
            {messages.length === 0 ? (
              <div className={styles.welcomeMessage}>
                <p>Hello! I'm Book Assistant, your AI assistant for Physical AI & Humanoid Robotics.</p>
