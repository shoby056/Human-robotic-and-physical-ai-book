...
const PersonalizedChapter1Page = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      ...
      const savedPersonalization = localStorage.getItem(`chapter-1-personalization-${user.username}`);
  ...
  const savePersonalization = (updatedData) => {
    if (user) {
      localStorage.setItem(`chapter-1-personalization-${user.username}`, JSON.stringify(updatedData));
    }
  };
...
export default PersonalizedChapter1Page;