...
const PersonalizedChapter2Page = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      ...
      const savedPersonalization = localStorage.getItem(`chapter-2-personalization-${user.username}`);
  ...
  const savePersonalization = (updatedData) => {
    if (user) {
      localStorage.setItem(`chapter-2-personalization-${user.username}`, JSON.stringify(updatedData));
    }
  };
...
export default PersonalizedChapter2Page;