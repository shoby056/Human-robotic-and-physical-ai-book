  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const botMessage = {
          id: Date.now() + 1,
          text: data.response || "I received a response but couldn't process it properly.",
          sender: 'bot',
        ...
        const errorDetail = errorData.detail || errorData.message || errorData.error || 'Unknown error';
        const errorMessage = {
          id: Date.now() + 1,
          text: `Sorry, I encountered an error: ${typeof errorDetail === 'string' ? errorDetail : JSON.stringify(errorDetail)}`,
          sender: 'bot',
          timestamp: new Date()
        };
      ...
      console.error('Failed to fetch from:', "http://localhost:8000/api/chat/chat");

      let errorMessageText = 'Sorry, I\'m having trouble connecting to the server. ';

      if (error.name === 'TypeError' && error.message.includes('fetch')) {
        errorMessageText += 'Please make sure the backend server is running on http://localhost:8000.';
      } else {
          ...
          <div className={styles.chatMessages}>
            {messages.length === 0 ? (
              <div className={styles.welcomeMessage}>
                ...
                <p><small><em>Note: Make sure the backend server is running on port 8000 for full functionality.</em></small></p>
              </div>
            ) : (
